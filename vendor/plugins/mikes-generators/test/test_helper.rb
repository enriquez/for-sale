...
tmp_dir = "#{File.dirname(__FILE__)}/../test/fixtures/tmp"
...
module MikesGenerators
  module TestHelper
    ...
    def assert_generated_factory_for(name)
      assert_generated_file("test/factories/#{name.to_s.underscore}.rb") do |body|
        assert_match /^Factory.define :#{name.to_s.singularize.underscore}/, body, "the file '#{name.to_s.underscore}.rb' should be a factory"
        yield body if block_given?
      end
    end
